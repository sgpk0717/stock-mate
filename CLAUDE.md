...
### 서비스 분리 (5컨테이너 + Redis IPC)

```
┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐
│  stockmate-api   │  │ stockmate-worker │  │  stockmate-mcp   │
│  (stateless)     │  │ (stateful)       │  │  (stateless)     │
│                  │  │                  │  │                  │
│  FastAPI :8007   │  │  live_runner     │  │  MCP SSE :8009   │
│  REST / WebSocket│  │  APScheduler 8잡 │  │  OpenClaw 연결   │
│  --reload 활성   │  │  장중 분봉 수집  │  │  24개 도구       │
│  텔레그램 발송큐 │  │  프로그램매매수집 │  │                  │
│                  │  │  알파 팩토리     │  │                  │
│                  │  │  인과 검증       │  │                  │
│                  │  │  일일 배치 수집  │  │                  │
...
**무중단 배포**: API 코드 수정 → uvicorn --reload 자동 재시작 → Worker/MCP는 안 죽음.
...
[키움 Open API+ (32-bit Python)]
        │ ZeroMQ PUB/SUB (< 1ms)
        ▼
[stockmate-api (FastAPI)]  ←── Redis IPC ──→  [stockmate-worker]
        │                                           │
        ├── REST API + WebSocket ──→ [React Frontend (Vite)]
        │                                           │
        ├── Claude API ──→ [AI Agent 시스템]        ├── Alpha Lab (진화적 알파 탐색)
        │                   ├── Manager Agent       │   ├── EvolutionEngine (DB 영속)
        │                   ├── Technical Analyst    │   ├── UCB1 연산자 9종
        │                   └── Risk Manager        │   ├── NumPy 인과 검증 + CPCV
        │                                           │   └── 벡터 메모리 (768-dim)
        │                                           │
[stockmate-mcp (MCP SSE)]  ←── Redis ──→           ├── Workflow FSM
        │                                           │   ├── APScheduler 8잡
        └── OpenClaw 크론잡 8개                     │   ├── FeedbackEngine
            ├── 07:00 모닝 브리프                    │   └── DivergenceDetector
            ├── 08:00 장 전 점검                     │
            ├── 12:00 장중 감시                      ├── Daily Scheduler
            ├── 16:00 장후 분석                      │   ├── 일봉/분봉/뉴스/수급
            ├── 18:15 마이닝 확인                    │   └── Circuit Breaker
            ├── 20:00 마이닝 리뷰                    │
            ├── 22:00 개선 검토                      ├── Simulation (ABM)
            └── 02:00 야간 점검                      │
                                                     │
                                                ┌────▼────┐
                                                │ KIS API │
...
- **API Server (stockmate-api):** FastAPI + uvicorn --reload. REST/WebSocket만 서빙 (stateless). 코드 수정 시 자동 재시작.
- **Worker (stockmate-worker):** 매매 세션(live_runner), APScheduler, 수집기, 알파 팩토리 (stateful). API 재시작 시에도 안 죽음.
- **MCP Server (stockmate-mcp):** FastMCP SSE. OpenClaw 크론잡 연결. API 재시작 시에도 안 죽음.
- **Redis (stockmate-redis):** API ↔ Worker ↔ MCP 간 IPC (Hash 캐시 + Stream 명령 큐 + Pub/Sub).
- **Frontend:** React + Lightweight Charts. 실시간 틱 차트 + 백테스트 + 자동매매 대시보드.

## 기술 스택

### Backend (`stock-mate-backend/`)
- Python 3.12, FastAPI 0.115, Uvicorn (--reload 개발 모드)
- SQLAlchemy 2.0 (async) + asyncpg, Alembic
- TimescaleDB (PostgreSQL 16 기반, Docker) — hypertable + continuous aggregates
- Redis 7 (IPC: Hash 상태 캐시 + Stream 명령 큐 + Pub/Sub 이벤트)
...
│   ├── app/
│   │   ├── main.py                    ← FastAPI 앱 (lifespan, WORKER_MODE 분기)
│   │   ├── worker.py                  ← Worker 엔트리포인트 (stockmate-worker 컨테이너용)
│   │   ├── core/
│   │   │   ├── config.py              ← pydantic-settings (DB/ZMQ/CORS/KIS/AI/뉴스/Redis)
│   │   │   ├── database.py            ← async engine + session (pool_size=10, max_overflow=20)
│   │   │   ├── redis.py               ← Redis 연결 풀 + IPC 래퍼 (hset/hgetall/xadd/xread/publish)
...
docker-compose up -d --build                                    # 전체 기동 (api + worker + mcp + redis + db)
...
curl http://localhost:8007/health                               # API 헬스체크
curl http://localhost:8009/sse                                  # MCP SSE 확인
docker exec stockmate-redis redis-cli ping                      # Redis 확인
```

### 개발 중 코드 반영 (무중단)
```bash
# API 코드 수정 → uvicorn --reload가 자동 재시작 (Worker/MCP 안 죽음)
# requirements.txt 변경 시에만:
docker-compose up -d --build --no-deps app

# Worker/MCP 코드 수정 시:
docker restart stockmate-worker   # Worker만 재시작
docker restart stockmate-mcp      # MCP만 재시작
```

### 서비스별 역할
| 컨테이너 | 포트 | 역할 | 재시작 영향 |
|----------|------|------|-----------|
| `stockmate-api` | 8007 | REST/WS API (stateless, --reload) | Worker/MCP 안 죽음 |
| `stockmate-worker` | — | 매매 세션 + 스케줄러 (stateful) | API/MCP 안 죽음 |
| `stockmate-mcp` | 8009 | MCP SSE (OpenClaw 연결) | API/Worker 안 죽음 |
| `stockmate-redis` | 6380 | IPC (Hash + Stream + Pub/Sub) | 거의 재시작 안 함 |
| `stockmate-db` | 5432 | TimescaleDB | 거의 재시작 안 함 |
...
| `REDIS_URL` | `redis://redis:6379/0` | Redis 연결 URL (Docker 내부) |
...
| `workflow_runs` | id(UUID), date, phase, status, config, mining_run_id, selected_factor_id, trading_context_id, review_summary, trade_count, pnl_amount, pnl_pct, step_status(JSON) | 일일 워크플로우 실행 |
...
| `telegram_message_logs` | id(UUID), category, caller, text, chat_id, status, error_message, telegram_message_id, created_at | 텔레그램 발송 기록 (자동 로깅) |
...
- **발송 로깅:** 모든 발송이 `telegram_message_logs` 테이블에 자동 기록 (category, caller, status)
- **로그 조회:** `GET /telegram/logs` (카테고리/상태 필터, 페이지네이션)
...
- [x] 텔레그램 발송 로깅 시스템 (telegram/models.py, schemas.py — 모든 발송 자동 기록)
- [x] REST API (routers/telegram.py — GET /telegram/logs, 카테고리/상태 필터)
...
- [x] WorkflowPage 전면 개선 (PhaseStepper + OpenClaw 크론잡 패널 + MCP 타임라인 + 실시간 매매 표시)

**프론트엔드 — 텔레그램 로그**
- [x] TelegramLogPage (발송 내역 조회 — 카테고리/상태 필터, 행 클릭 펼침, 페이지네이션)

**서비스 분리 + Redis IPC (2026-03-18~19)**
- [x] Phase 0: Redis 인프라 (docker-compose에 redis:7-alpine, app/core/redis.py)
- [x] Phase 1: Worker 상태 Redis 이중 기록 (live_runner + orchestrator → Redis Hash)
- [x] Phase 2: API가 Redis에서 읽기 (workflow/status, trading/status → Redis 우선, DB 폴백)
- [x] Phase 3: MCP 서버 분리 (stockmate-mcp 독립 컨테이너, Redis 경유)
- [x] Phase 4: 명령 큐 (Redis Stream commands:workflow → Worker 소비)
- [x] Phase 5: Worker 분리 (stockmate-worker, WORKER_MODE=external)
- [x] Phase 6: --reload 적용 (API 코드 수정 시 자동 재시작, Worker/MCP 무관)

**버그 수정 + 개선 (2026-03-18)**
- [x] g50 팩터 재평가 (turnover=0, Sharpe 3.6배 과대평가 → 정상화)
- [x] orchestrator UTC→KST 버그 수정 (_set_step, _phase_watchdog)
- [x] watchdog 세션 자동 복구 (컨테이너 재시작 시 5분 내 세션 재생성)
- [x] live_runner 분봉 수집 분리 (장중 수집기 독립 Task)
- [x] live_runner 데이터 로딩 to_thread (이벤트 루프 해방, API 응답 15초→0.5초)
- [x] 텔레그램 발송 큐 + rate limiter (429 방지, 1.5초 간격)
- [x] 텔레그램 일일 리뷰: Claude LLM 생성 + 세션별 분리 성과
- [x] trade_reviewer context_id 필터 + pnl_pct 자본금 기준 계산
- [x] DB pool_size 5→10, max_overflow 10→20
- [x] MCP send_and_log_telegram 도구 추가
- [x] OpenClaw 크론잡 프롬프트에 log_telegram_message 지시 추가
- [x] live_runner 중복 봉 감지 + 스킵 (디버깅 로그)
- [x] WorkflowPage step_status 키 매핑 (TRADING→market_open) + 값 파싱 (객체→문자열)
- [x] WorkflowPage 텔레그램 메시지 HTML 태그 제거 (stripHtml)
